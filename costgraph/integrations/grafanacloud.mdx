...
- In Grafana Cloud, create an **Access Policy** whose realm is the organization, with
  the `billing-metrics:read`, `org-billing-info:read`, `org-billing-rate:read`,
  `invoices:read`, and `orgs:read` scopes. Create a token for it.
- The token must be an organization access policy token, which starts with `glc_`. A
  stack service account token, which starts with `glsa_`, can't read billing.
- Note the organization slug, the `{org}` in `grafana.com/orgs/{org}`.
...
3. Enter the access policy token.
4. Select **Connect**.
