...
| `OPENAI_TENANT_GROUP` | no | Set to `project_id` to split spend per project into the FOCUS sub-account columns (adds `project_id` to the cost `group_by`). Any other value is rejected. See [Guide](/costgraph/focus-exporter/tenancy). |
