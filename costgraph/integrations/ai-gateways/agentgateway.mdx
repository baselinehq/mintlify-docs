...
<Note>
  ...
  `targetSelectors`) naming your Gateway, and it must live in the same namespace
  as that Gateway - a policy cannot reach one in another namespace. If you
  already have a policy targeting it, add the `attributes` block to that one
