...
description: "Bring your Cherry Servers bare-metal spend into CostGraph, pulled by us or pushed by you"
---
...
- In the Cherry Servers dashboard, go to **API keys** and create a key.
- Note the team id if the key can see more than one team. Leave it empty and CostGraph
  reads every team the key reaches.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Cherry Servers**.
2. Name the connection and pick the tenant these servers belong to.
3. Enter the API key, and a team id if you want one team instead of every team the
   key reads.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Cherry Servers on
a schedule and pushes the records for you, wherever you want to run it: a container, a
Kubernetes `CronJob`, or a binary on a machine you already have.

```bash
focus-exporter --provider cherryservers --sink costgraph
```

Your Cherry Servers credential stays where you put it, and only the finished FOCUS
