...
- In Vercel, go to **Account Settings > Tokens** and create a token with a role that
  reads billing for the team you want.
- If the token sees more than one team, note the team to export.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Vercel**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Vercel needs and marks the
   optional ones.
4. Select **Connect**.
...
```bash
focus-exporter --provider vercel --sink costgraph
```
