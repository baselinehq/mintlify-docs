...
The CostGraph Operator runs in your Kubernetes cluster and continuously reports its state to CostGraph. It deploys three components:
...
- **Operational/Compute Metrics scraper**: scrapes cluster, host, and container metrics and remote-writes them to CostGraph.
- **Network Metrics scraper**: scrapes container network metrics and, optionally, host-process network metrics, then remote-writes them to CostGraph.

Together they collect:

- **Cluster inventory**: the live manifests of all resources in the cluster.
- **Container CPU/memory metrics**: per-container CPU and memory from cAdvisor.
- **Container and host process network metrics**: per-container and, optionally, per-process bytes sent/received, packets sent/received, new-connection counts.
...
- **GPU metrics**: optional, from the NVIDIA DCGM exporter. See [Accelerators](/costgraph/operator/accelerators).
...
- **Idle detection**: workloads that consistently use less than they request.
...
<Steps>
  ...
  <Step title="Verify">
    ...
    After all pods are `Running`, your cluster appears in the [CostGraph platform](https://app.costgraph.ai/).
  </Step>
</Steps>

<Note>
  Running GPU nodes? Enable the DCGM exporter for GPU metrics. See [Accelerators](/costgraph/operator/accelerators).
</Note>
