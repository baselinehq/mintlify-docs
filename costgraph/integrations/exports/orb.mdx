...
before reporting it, then keeps reconciling the last 35 days. Orb bills every
event into the customer's **current** period, so a correction to an old day
shows up on the next invoice - `usage_date` is how you and your customer see
which day it belongs to. Orb's own backfill flow is the way to move a
correction into a closed period.
</Note>
