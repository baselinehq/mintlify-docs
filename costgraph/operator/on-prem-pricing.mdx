...
|--------|-------------------|---------------|
| `.compute` | compute rate | [`POST /v1/marketplace/compute`](https://docs.costgraph.ai/api-reference/marketplace/publish-compute-prices) |
| `.storage` | disk rate | [`POST /v1/marketplace/disks`](https://docs.costgraph.ai/api-reference/marketplace/publish-disk-prices) |
| `.database` | managed database rate | [`POST /v1/marketplace/databases`](https://docs.costgraph.ai/api-reference/marketplace/publish-database-prices) |
| `.model` | model token rate | [`POST /v1/marketplace/models`](https://docs.costgraph.ai/api-reference/marketplace/publish-model-prices) |
...
[Publish compute prices](https://docs.costgraph.ai/api-reference/marketplace/publish-compute-prices)
for compute, and
[Publish disk prices](https://docs.costgraph.ai/api-reference/marketplace/publish-disk-prices)
for storage.

```bash
curl -X POST https://pricing.baselinehq.cloud/v1/marketplace/compute \
...
```bash
curl -X POST https://pricing.baselinehq.cloud/v1/marketplace/disks \
  -H "Authorization: Bearer $COSTGRAPH_API_KEY" \
  -H "Content-Type: application/json" \
