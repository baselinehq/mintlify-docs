...
A single entry from `aggregate=controller` over a two-day window, with the
volume and network cost the workload carries:

```json
{
  "name": "statefulset:tsdb-vmstorage-long",
  "properties": { "controller": "tsdb-vmstorage-long", "controllerKind": "StatefulSet" },
...
Three volumes, each billed $0.20, add to the `pvCost` of $0.60. An
internet-facing workload reads the same way with `loadBalancerCost` filled in.
The ingress controller on this cluster carries $0.92 of load balancer and $0.22
of network cost against $0.01 of CPU and memory. That is why the four are
reported separately.
...
| `gpuCost`, `gpuCount`, `gpuHours` | Always `0`. CostGraph prices GPU instance types but does not yet meter GPU use per container, so GPU cost stays in the node's bill and lands in idle. |
| `networkTransferBytes`, `networkReceiveBytes` | Always `0`. `networkCost` is filled in; the byte counters behind it are not carried on the allocation. |
| `lbAllocations` | Always `null`. `loadBalancerCost` is filled in, without the per-Service breakdown. |
