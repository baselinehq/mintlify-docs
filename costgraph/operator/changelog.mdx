...
<Update label="2026-08-28" description="costgraph-operator v0.5.0">
    ### Breaking Changes
    - **`helm upgrade` fails until the two operator Deployments are deleted.** `costgraph-operator-kubernetes` and `costgraph-operator-prometheus` are now rendered by a shared chart, and their `spec.selector` changes as a result. A Deployment's selector is immutable, so Kubernetes rejects the patch rather than replacing the object. Delete both before upgrading; they are stateless and are recreated immediately:
      ...
      No metrics are lost. The operators resume from the cluster's current state on restart, and the collectors that hold data are DaemonSets, which are unaffected.
    - **Values under `operatorKubernetes` and `operatorPrometheus` move beneath `deployment`.** A values file that sets them at the old paths still parses, but the settings no longer apply and the chart's defaults are used instead. Nothing errors, so check any override you rely on:
      ...
      The same mapping applies to `operatorPrometheus`. Settings under `config`, and the top-level `global` and `rbac` values, are unchanged.

    ### Updates
    - The two operator Deployments are rendered from a shared definition rather than a template each, so they no longer drift apart in the fields that should match
    - The flowtrace DaemonSet, the ClusterRole and its bindings, the ServiceAccount, the credentials Secret and every bundled exporter are unchanged
</Update>
