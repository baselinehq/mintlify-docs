...
| `DIGITALOCEAN_ACCOUNT_ID` | no | Explicit billing-account id. When empty the adapter resolves it from `/v2/account`: a team gives its UUID and name, and an individual account (no team) uses the account UUID for both the id and the name. The account email is never used as a label, so no personal email lands in FOCUS output. If that call is not accessible (e.g. a billing-only token) the export fails and asks you to set this, rather than inventing an id. |
...
| `BillingAccountId` / `BillingAccountName` | the team UUID/name from `/v2/account`, or the account UUID (used for both) when there is no team, or `DIGITALOCEAN_ACCOUNT_ID` when set; the account email is never used as a label |
