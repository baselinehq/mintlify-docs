...
<CardGroup cols={3}>
  ...
  <Card title="One FOCUS dataset" icon="layer-group">
    No per-provider views to learn. Cross-provider comparison is a `GROUP BY`.
  </Card>
</CardGroup>
...
flowchart LR
  ...
  A -->|daily pull| N[FOCUS 1.2 normaliser]
  B -->|daily read| N
  C -->|POST /tenant/billing/focus| N
  N --> R[Raw billing rows]
  R --> L[Cost line items]
  L --> G[Daily cost by day x service x resource]
  D --> G
...
Every provider lands as raw records first, so a normalisation change is a replay, not
a re-fetch. Records are then turned into line items and rolled up into daily cost.
Kubernetes and host telemetry join the same rollup, which is what lets a node bill be
...
Read from a native FOCUS export rather than a billing API, so the grain is whatever
the export carries.
...
<CardGroup cols={2}>
  ...
  <Card title="Anomaly detection" icon="triangle-exclamation">
    Deterministic detectors run hourly over the same rollup, in and across providers.
  </Card>
