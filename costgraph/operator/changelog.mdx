...
sidebar_position: 1
title: Changelogs
description: "Release history for the CostGraph operator"
---
...
<Update label="2026-09-26" description="costgraph-operator v0.7.2">
    ### Breaking Changes
    - N/A

    ### Updates
    - **The AI gateway scraper reports latency, capacity and configuration bindings again. Upgrade to get them back.** All three `costgraph-operator-prometheus` images move to `v0.5.1`. Two were pinned to a build identifier and one to `v0.5.0`, all of them one commit behind the usage contract that carries those fields. The scraper had no place to put them, so it dropped them while reading each payload. Token usage kept arriving, because the metric bindings name it explicitly. Latency and capacity stopped without an error anywhere. Any release still on an older tag is missing those signals today, and nothing in the cluster reports a failure.
    - `costgraph-operator-kubernetes` moves from `v0.1.1` to `v0.1.2`, rebuilt on Go 1.26.6. The rebuild clears the standard library advisories that applied to the previous build, so upgrade even if you need nothing else in this release.
    - **New accelerator exporter, shipped off by default.** A DaemonSet attributes per-process accelerator usage to the pod and container that owns it, and serves those metrics on port `9405`. Turn it on by upgrading with the values you installed with, because `helm upgrade` returns anything you do not pass back to the chart defaults:

      ```sh
      helm upgrade <release> costgraph/costgraph-operator \
        -f your-values.yaml \
        --set gpuExporter.enabled=true
      ```

      It reads the host driver from `gpuExporter.driverInstallDir`, which defaults to the GKE path `/home/kubernetes/bin/nvidia`. Set it to your own driver directory on any other distribution. The pod runs as root on accelerator nodes only, and drops every capability.

      Processes on partitioned cards are attributed to the partition they run in. One gap remains. A process the host owns on a partition is not attributed to a card. Nothing yet maps a partition back to its physical accelerator. Pod and container attribution is unaffected.
    - Chart `0.7.2` changes no other image, value or object.
</Update>

<Update label="2026-08-28" description="costgraph-operator v0.5.0">
    ### Breaking Changes
    - **`helm upgrade` fails until the operator Deployments are deleted.** `costgraph-operator-kubernetes`, `costgraph-operator-prometheus`, `costgraph-operator-ai-gateway` and the `costgraph-operator-flowtrace` DaemonSet are now rendered by a shared chart, and their `spec.selector` changes as a result. A selector is immutable, so Kubernetes rejects the patch rather than replacing the object. Delete them before upgrading; they are stateless and are recreated immediately:

      ```sh
      kubectl -n <namespace> delete deployment \
        <release>-costgraph-operator-kubernetes \
        <release>-costgraph-operator-prometheus \
        <release>-costgraph-operator-ai-gateway
      kubectl -n <namespace> delete daemonset \
        <release>-costgraph-operator-flowtrace
      helm upgrade <release> costgraph/costgraph-operator
      ```

      Omit the ones you do not run. `aiGatewayScraper` is off by default. No metrics are lost: the operators resume from the cluster's current state on restart.

    - **Objects no longer carry the release name.**
      ...
      Each component's objects are named from its `applicationName`. `fullnameOverride` no longer reaches them: it now affects only the ClusterRole (`<fullname>-view`), the ClusterRoleBinding and the ServiceAccount.

    - **Every component's workload settings move beneath a `deployment:` or `daemonSet:` block.** A values file that sets them at the old paths still parses, but the settings no longer apply and the chart's defaults are used instead. Nothing errors, so check any override you rely on:

      | Before | After |
      | --- | --- |
      | `operatorPrometheus.image.tag` | `operatorPrometheus.deployment.image.tag` |
      | `operatorPrometheus.replicas` | `operatorPrometheus.deployment.replicas` |
      | `operatorPrometheus.resources` | `operatorPrometheus.deployment.resources` |
      | `operatorPrometheus.env` (list) | `operatorPrometheus.deployment.env` (map) |
      | `operatorPrometheus.securityContext` | `operatorPrometheus.deployment.securityContext` |
      | `operatorPrometheus.nodeSelector` / `.tolerations` / `.affinity` | the same key under `operatorPrometheus.deployment` |
      | `operatorPrometheus.podAnnotations` / `.podLabels` | the same key under `operatorPrometheus.deployment` |

      The same mapping applies to `operatorKubernetes` and `aiGatewayScraper`. For `flowtrace` the block is `daemonSet:` rather than `deployment:`, and it takes `image`, `resources`, `priorityClassName`, `tolerations` and the scheduling keys.

      `enabled` stays at component level on all four. `<component>.deployment.enabled` (or `.daemonSet.enabled`) is a second, narrower switch that keeps the configuration and drops the pods. `flowtrace.maxUnavailable` and `operatorPrometheus.scrapeTargets` also stay at component level. Settings under `config`, and the top-level `global` and `rbac` values, are otherwise unchanged.

    - **`global.namespace` is removed.** It was read by a helper looking at a path that did not exist, so setting it never had an effect. The release namespace is used.

    - **`env` is now a map, not a list.** `env.LOG_LEVEL.value: info` rather than `- name: LOG_LEVEL`. A map means `--set` can override one variable without clobbering its siblings. A list at the new path is not read.

    ### Updates
    - All four CostGraph components (`operatorKubernetes`, `operatorPrometheus`, `aiGatewayScraper` and `flowtrace`) are rendered from one shared definition and take one values shape, and enabling, sizing or scheduling any of them is the same operation. The same contract covers the `costgraph-selfhosted` chart; see the [values reference](/costgraph/self-hosted/values)
    - `operatorPrometheus` and `aiGatewayScraper` gain a `pprof` container port declaration, and `aiGatewayScraper` gains an explicit `replicas`
    - `flowtrace` withholds its service account token by default, keeping a privileged per-node pod off the chart's cluster-wide read ClusterRole
    - The ClusterRole and its bindings, the ServiceAccount, the RBAC values and every bundled exporter are unchanged
</Update>

<Update label="2026-08-17" description="costgraph-operator v0.3.0">
    ### Breaking Changes
...
<Update label="2026-08-11" description="costgraph-operator v0.2.3">
    ### Breaking Changes
    - N/A
...
<Update label="2026-04-12" description="costgraph-operator v0.1.3">
    ...
    <Note>
        With this release the `costgraph` chart is superseded. New installations must use the `costgraph-operator` chart.
    </Note>
</Update>
