...
- In the Redis Cloud console, go to **Account Settings > API** and enable the API. This
  gives you the account key.
- Under **API keys**, create a user key. Both halves are required.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Redis Cloud**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the account key and the user key. The account needs the Owner, Billing
...
```bash
focus-exporter --provider rediscloud --sink costgraph
```
