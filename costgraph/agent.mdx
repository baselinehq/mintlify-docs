...
- **[Rightsizing recommendations](/costgraph/recommendations)** — resize suggestions from observed CPU and memory usage, including cheaper instance types across cloud providers.
- **[Idle detection](/costgraph/recommendations#idle-detection)** — VMs with consistently low CPU, memory, network, and disk activity are flagged to terminate.
...
<Tabs>
  <Tab title="Script" icon="terminal">
    ...
    Pin a specific release with `COSTGRAPH_AGENT_VERSION`, for example `COSTGRAPH_AGENT_VERSION=v0.0.64`.
  ...
  <Tab title="Ansible" icon="server">
    ...
    <Steps>
      <Step title="Install the collection">
        ```bash
        ansible-galaxy collection install costgraph.agent
        ```
      </Step>
      ...
      <Step title="Run it against your inventory">
        ...
        The role installs the binary at `/usr/local/bin/costgraph-agent`, writes the API key to `/etc/default/costgraph-agent`, and manages the `costgraph-agent` systemd service. Pin a release with the `costgraph_agent_version` variable.
      </Step>
    </Steps>
  </Tab>
</Tabs>
