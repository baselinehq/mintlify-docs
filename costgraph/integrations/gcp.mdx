...
```sh
curl -sSL https://setup.costgraph.ai/integrations/gcp.sh | COSTGRAPH_EXTERNAL_ID="org_..." sh
```

Cloud Shell already carries your console credentials, so no key is created or pasted
anywhere. The script resolves your billing account, creates the reader service account,
finds the FOCUS and detailed datasets, applies the grants below, and authorizes
CostGraph's federated principal to impersonate it. It is idempotent - rerun it safely.
If the FOCUS export does not exist yet it stops and points you back at Step 1.

<Note>
The billing account is resolved for you when your account has exactly one. With several,
the script lists them and asks you to pick:

```sh
curl -sSL https://setup.costgraph.ai/integrations/gcp.sh \
  | COSTGRAPH_EXTERNAL_ID="org_..." COSTGRAPH_BILLING_ACCOUNT_ID="01F81F-B1B36E-2FAB46" sh
```

The id is the **Account ID** column at
...
BILLING_ACCOUNT_ID=01F81F-B1B36E-2FAB46
FOCUS_DATASET=gcp_billing_immutable_01F81F_B1B36E_2FAB46_us
DETAILED_DATASET=gcp_billing_export_resource_v1
```
...
2. Paste the values the Cloud Shell script printed. It fills the service account, query
   project, and billing account for you. To set it up by hand instead, choose
