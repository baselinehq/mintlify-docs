...
One Cloud Shell script creates the storage and the read-only credential. Like Google
Cloud, turning the billing export on is a one-time portal step: the exports API only
accepts the classic dataset types, not FOCUS.
...
The script ends by reminding you of the one manual step: create the FOCUS export in
the portal (Step 2 below).

Set `COSTGRAPH_SUBSCRIPTION_ID`, `COSTGRAPH_RESOURCE_GROUP`, `COSTGRAPH_LOCATION`,
`COSTGRAPH_STORAGE_ACCOUNT`, or `COSTGRAPH_CONTAINER` to override the defaults.

## Step 2: Create the FOCUS export (portal, one-time)

<Note>
Enabling a FOCUS export is **portal-only** - the exports REST API accepts only the
classic `ActualCost` and `AmortizedCost` dataset types, not FOCUS. This is the one
manual step.
</Note>

1. Open [Exports in the Azure portal](https://portal.azure.com/#view/Microsoft_Azure_CostManagement/Menu/~/exports/open/exports/openedBy/BillingAccount.Exports.ScopePicker)
   (**Cost Management + Billing -> Exports**) and pick your billing account.
2. Click **+ Create** and choose the **Cost and usage details (FOCUS)** dataset.
3. Choose **Daily** execution, month-to-date, with overwrite.
4. Destination: the storage account, container and folder the script printed
   (`cgfocus...` / `focus-exports` / `costgraph-focus`).

FOCUS exports need an EA or MCA billing scope; on a pay-as-you-go MOSP subscription
the dataset is not offered. The first files can take up to 24 hours to appear, so if
the connect check finds nothing to read yet, wait for the first files and connect
again.
