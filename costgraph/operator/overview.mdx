...
- **Network Metrics scraper** - scrapes container and,optionally, host-process network metrics and remote-writes them to Costgraph.
...
- **Container and host process network metrics** - per-container and, optionally, per-process bytes sent/received, packets sent/received, new connections count per-container and per-process.
...
- For network flow tracing, a BTF-enabled kernel and a host-mounted cgroup v2 filesystem
