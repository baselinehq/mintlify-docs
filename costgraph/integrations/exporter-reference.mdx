...
Grain: day x resource x service, as the FOCUS export publishes it.
Reads the current month by default; pass `--month` or `--start`/`--end` for another window.

Provider documentation: https://docs.costgraph.ai/costgraph/integrations/azure

| Variable | Required | Secret | Description |
| --- | --- | --- | --- |
| `AZURE_TENANT_ID` | yes | no | Tenant id of the Entra ID app (service principal) reading the export. Shown as the directory ID on the app's Overview page. |
| `AZURE_CLIENT_ID` | yes | no | Client id (application id) of the service principal. The Cloud Shell setup script creates it with Storage Blob Data Reader on the export's storage account. |
| `AZURE_CLIENT_SECRET` | yes | yes | Client secret of the service principal. Shown once when created. |
| `AZURE_STORAGE_ACCOUNT` | yes | no | Storage account the FOCUS export writes to, from Cost Management + Billing -> Exports. |
| `AZURE_CONTAINER` | yes | no | Container inside the storage account that holds the export. |
| `AZURE_EXPORT_NAME` | no | no | Folder the export writes into. Leave empty to read everything in the container; only set it to read one export among several. |

## Baseten

`--provider baseten` (AI / LLM)

Grain: day x model x token type (model APIs), day x deployment (dedicated), day x job (training).
Reads the current month by default; pass `--month` or `--start`/`--end` for another window.

Provider documentation: https://docs.costgraph.ai/costgraph/integrations/baseten

| Variable | Required | Secret | Description |
| --- | --- | --- | --- |
| `BASETEN_API_KEY` | yes | yes | Baseten API key with organization scope. Create one under Settings -> API keys; it reads the whole organization's billing, not just your own. |
...
| `BLACKSMITH_ORG` | yes | no | The GitHub organization login exactly as it appears on GitHub, case-sensitive (e.g. AcmeCorp, not acmecorp). This is the org whose CI usage is exported. |
...
`--provider fireworks` (AI / LLM)

Grain: day x model x token type (serverless), day x deployment x accelerator (dedicated), plus a quota snapshot.
Reads the current month by default; pass `--month` or `--start`/`--end` for another window.

Provider documentation: https://docs.costgraph.ai/costgraph/integrations/fireworks

| Variable | Required | Secret | Description |
| --- | --- | --- | --- |
| `FIREWORKS_API_KEY` | yes | yes | Fireworks API key. Create one under Account Settings -> API Keys. The user it belongs to needs the account administrator role, otherwise the cost query returns only that user's own spend. |
| `FIREWORKS_ACCOUNT_ID` | yes | no | Account slug the usage is billed to, shown in the console URL and in firectl output. |
...
`--provider microsandbox` (Compute)

Grain: day x sandbox, day x volume, plus a quota snapshot.
Reads the current month by default; pass `--month` or `--start`/`--end` for another window.

Provider documentation: https://docs.costgraph.ai/costgraph/integrations/microsandbox

| Variable | Required | Secret | Description |
| --- | --- | --- | --- |
| `MICROSANDBOX_API_KEY` | yes | yes | Organization API key (msb_ak_...), created in the microsandbox console. It is org-scoped and reads the whole organization's usage. |
...
Provider documentation: https://docs.costgraph.ai/costgraph/integrations/objectstore
...
| `OBJECTSTORE_ACCESS_KEY_ID` | no | yes | Access key with read permission on the bucket. On Azure Blob Storage, a service principal instead: "<tenant id>/<client id>" of an app granted Storage Blob Data Reader. Leave empty on AWS to use the role or profile the exporter already runs as. |
| `OBJECTSTORE_SECRET_ACCESS_KEY` | no | yes | Secret for the access key, or the client secret of the service principal on Azure Blob Storage. |
