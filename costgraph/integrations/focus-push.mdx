...
| `Provider` | Non-empty, or send `ProviderName` instead. Every row in a batch must agree, and must match the connection. |
...
A vendor that publishes FOCUS rarely publishes it exactly, and the deviations are
usually repairable. CostGraph repairs these on the way in rather than refusing the
batch:

| Field | What happens |
|-------|--------------|
| `ChargeDescription` | Null or absent is filled from `ServiceName`. The spec allows it to be null. |
| `ServiceCategory` | A value outside the FOCUS enum is mapped onto it - `Compute Engine` and `Runtime` become `Compute`, `Logging & Monitoring` becomes `Management and Governance`. A value nothing matches becomes `Other`, so the charge still counts. |
| `Provider` | Taken from a `ProviderName` column when `Provider` itself is absent, which is how most vendor exports name it. |

Your own category is not discarded. Unless you set `ServiceSubcategory` yourself, the
value you declared is kept there, so a row filed under `Compute` still says
`Compute Engine GPU` in breakdowns.

<Note>
The FOCUS `ServiceCategory` enum is `AI and Machine Learning`, `Analytics`,
...
net, not a substitute for saying what you mean.
</Note>
