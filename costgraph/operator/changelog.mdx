...
<Update label="2026-08-28" description="costgraph-operator v0.5.0">
    ...
    - **Objects no longer carry the release name.** The chart is installed once per namespace, so the prefix distinguished nothing, and components rendered by the shared application chart cannot reproduce it.
    ...
    ### Updates
    - All four CostGraph components - `operatorKubernetes`, `operatorPrometheus`, `aiGatewayScraper` and `flowtrace` - are rendered from one shared definition and take one values shape, so they no longer drift apart in the fields that should match, and enabling, sizing or scheduling any of them is the same operation. The same contract covers the `costgraph-selfhosted` chart; see the [values reference](/costgraph/self-hosted/values)
