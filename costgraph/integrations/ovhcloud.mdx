...
- Copy both values from the confirmation window. The **service account name** is the
  client id and the **password** is the client secret. OVHcloud displays the secret only at
  creation and can't retrieve or regenerate it, so a lost secret means a new service
  account.
