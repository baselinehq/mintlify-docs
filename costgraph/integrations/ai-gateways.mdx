...
<AccordionGroup>
  ...
  <Accordion title="Solo.io agentgateway">
    Add a metric field computed from the request. The value is a CEL expression,
    so the identifier can come from a header, a JWT claim, or anything else in
    the request.
    ...
    config:
      metrics:
        fields:
          add:
            customer: 'request.headers["x-customer-id"]'
    ```
  </Accordion>
