...
`flowtrace` scrapes per-container, per-process network metrics and remote-writes them to Costgraph.
...
<ParamField body="flowtrace.config.includeHostFlows" type="boolean" default="false">
  Collect host-process network flows alongside container network flows.
</ParamField>
...
The chart pulls in the metrics sources as subcharts. Disable any you already run in the cluster. For flowtrace, disable if network tracing isn't a need.
