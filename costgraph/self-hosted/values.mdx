...
<ParamField body="global.controlPlane.existingSecret" type="string">
  ...
  over `apiKey`. Requires `global.imagePullSecrets`, since the chart cannot read
...
Both charts assume one release per namespace, so the release-name prefix
distinguished nothing - and components rendered by `application` cannot
reproduce it. These are live objects being renamed, so plan the upgrade.
