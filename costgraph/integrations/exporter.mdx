...
<Tabs>
  <Tab title="Container">
    ```bash
    docker login registry.costgraph.ai -u x -p "$COSTGRAPH_API_KEY"
    ...
    docker run --rm \
      ...
      registry.costgraph.ai/baselinehq/focus-exporter:latest \
      --provider objectstore --sink costgraph --month 2026-08
    ```

    The container runs once and exits. Schedule it with cron, a systemd timer,
  ...
  <Tab title="Binary">
    ...
    focus-exporter --provider objectstore --sink costgraph --month 2026-08
    ```

    Writing the records to a file instead of pushing them is a good way to see
    what would be sent: drop `--sink costgraph`.
  </Tab>
</Tabs>
...
[provider reference](https://github.com/baselinehq/focus-exporter/blob/main/docs/providers/environment.md).
...
`OBJECTSTORE_LAYOUT` says how the provider arranges the files. Prefer
`manifest`, which reads exactly the files the export names - AWS and Azure both
publish one, so nothing is inferred and a run that lands while the exporter is
reading cannot be read half-written. Use `latest-run` for a provider that
rewrites a whole billing period every run, such as STACKIT, and `flat` when the
bucket holds one copy of each file.
