...
<Tabs>
  ...
  <Tab title="Cloud-init" icon="cloud">
    ...
    <Tabs>
      <Tab title="AWS">
        Add it under User data on the instance or launch template (in the EC2 console, expand Advanced details to reach User data; or `aws ec2 run-instances --user-data file://user-data.sh`). Instances launched from that template by an Auto Scaling group install the agent automatically.
      </Tab>
      <Tab title="GCP">
