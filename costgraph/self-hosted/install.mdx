...
under [**Settings > API keys**](https://app.costgraph.ai/settings/account/api-keys).
It is the only credential you need. The image registry authenticates with the
same key, and the chart wires that up for you.

Self-hosting still starts with an account on
[app.costgraph.ai](https://app.costgraph.ai). Sign up there, and that account is
where you create the deployment API key, pick your plan, and manage your
subscription and billing. The account is for provisioning only: your cost data
never leaves the deployment you run.
...
<Warning>
  ...
  runs. A database holding your cost history must outlive a Helm release.
</Warning>
...
<Accordion title="What is a pricing ID and why would I need it?">
  ...
  actually costs you. You then tag it onto the resources that bill at it.
  Without one, hardware you own is measured but has nothing to price against.
...
### If the deployment can't bootstrap

On startup the backend registers with CostGraph and receives a signed
configuration document. That document requires an active license on your
organization, so bootstrap fails while your plan is still being set up:

```
no active license for this organization
a deployment cannot bootstrap without one
```

Sign in at [app.costgraph.ai](https://app.costgraph.ai) and check that the
organization holding your deployment API key has an active plan. If it does and
the error persists, send the diagnostic bundle to [support@costgraph.ai](support@costgraph.ai).

Sign-in on your deployment depends on the same document, because the Cognito
user pool it names arrives inside it. A deployment that hasn't bootstrapped
reports `Auth UserPool not configured` on the sign-up form. That message means
the configuration hasn't arrived yet, not that you're missing a setting: there
is no Cognito value for you to fill in. Fix the bootstrap and sign-in works.
