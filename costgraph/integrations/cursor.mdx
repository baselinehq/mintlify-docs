...
- In the Cursor dashboard, go to **Settings > Team > API Keys** and create an admin API
  key. The key is available on Team and Enterprise plans.
- Note your team name. The events API returns no team id, so CostGraph uses the name
  you enter as the billing account label.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Cursor**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Cursor needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Cursor on a
...
```bash
focus-exporter --provider cursor --sink costgraph
```

Your Cursor credential stays where you put it, and only the finished FOCUS records
