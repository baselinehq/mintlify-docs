...
- In the Anthropic Console, go to **Settings > Admin keys** and create an admin key.
  Only an organization admin can create one, and only an admin key reads
  organization-wide usage and cost.
- Check that the organization is a Claude Console organization. Claude Enterprise
  organizations expose a separate Analytics API that an admin key can't authenticate
  against.
...
3. Enter the values you copied. The form lists what Anthropic needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Anthropic on a
...
```bash
focus-exporter --provider anthropic --sink costgraph
```

Your Anthropic credential stays where you put it, and only the finished FOCUS records
