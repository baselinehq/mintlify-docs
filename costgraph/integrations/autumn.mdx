...
1. In your Autumn dashboard, open **Developer -> Secret keys** and copy a key.
   Use a live key to bill for real; a sandbox key exercises the whole flow
   without invoicing anyone.
2. In CostGraph, open **Integrations** and choose **Autumn**.
3. Paste the key and connect. CostGraph verifies the key against Autumn before
...
before binding it to a meter, and watch the report history after rule changes.
</Warning>
...
- **Resync** on the connection runs a report immediately.
- A day is held back rather than reported wrong when its spend mixes
  currencies, or a quantity meter meets an unexpected unit. Held days and the
  reason are visible in the report history.
