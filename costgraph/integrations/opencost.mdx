...
Kubernetes Service instead. `kubectl cost` is one of them: it resolves a Service
and port rather than calling a host, so it needs one to exist.
...
`kubectl cost` then reads CostGraph. Pass `--kubecost-namespace` when the
operator isn't in a namespace called `opencost`. Pass `--historical` to see what
the window cost, rather than the monthly rate the plugin projects by default:
...
Leave it off on a cluster still running OpenCost, which already has a Service by
that name.
