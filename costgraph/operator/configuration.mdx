...
Every component in this chart follows one values shape. A component is a
top-level key; its workload lives in a block named after the Kubernetes kind it
produces (`deployment:` for a Deployment, `daemonSet:` for a DaemonSet), and the
inner keys - `replicas`, `image`, `resources`, `env`, `ports`, `nodeSelector`,
`tolerations`, `affinity` - mean the same thing in each. Enabling, sizing or
scheduling any component is therefore the same operation.
