...
- An API key. Create one in **Settings > API keys**. Allocation and assets need
  the `operator:read` scope; cloud cost needs `focus:read`. Grant both to read
  every route with one key.
...
port, in the namespace you install the release into. Install the operator into
`opencost` and the plugin finds it with no flags:

```shell
kubectl cost namespace --opencost --window 1d --historical
```
...
<Warning>
`--opencost` sets the namespace along with the name, port, and path, and it wins
over `--kubecost-namespace` whatever order you write them in. To read a release
installed anywhere else, set the four values yourself instead of passing
`--opencost`:
</Warning>
...
Change `--service-name` and `--service-port` to match if you also change
`openCostApi.service.name` or `.port`.
...
Install into the `kubecost` namespace and `kubectl cost` needs no flags at all:

```shell
kubectl cost namespace --window 1d --historical
```

Elsewhere, `--kubecost-namespace` is enough on its own. The Kubecost defaults
are not bundled behind a flag, the way `--opencost` bundles them:

```shell
kubectl cost namespace --kubecost-namespace costgraph --window 1d --historical
```
...
The `namespace`, `controller`, `deployment`, `pod`, `node`, and `label` views
all read CostGraph. `kubectl cost predict` and `kubectl cost tui` do not:
prediction is a Kubecost endpoint CostGraph does not serve.
...
`aggregate` accepts `cluster`, `node`, `namespace`, `controller`,
`controllerKind`, `pod`, `container`, and `label:<key>`. CostGraph composes
names the way OpenCost does, joining the properties you asked for with `/`. A
controller carries its kind and name, so `aggregate=controller` returns
`deployment:api`. A property with no value returns `__unallocated__`.

To group by one kind of controller, ask for it by name: `deployment`,
`statefulset`, `daemonset`, `job`, or `cronjob`. Each returns the controllers of
that kind and gathers everything else under `__unallocated__`, so the cost still
adds up to the same total.

`label:<key>` groups by a pod label. `aggregate=label:team` returns one
allocation per team, and pods without the label land in `__unallocated__`.
Combine it with anything else: `aggregate=namespace,label:app`.

<Note>
CostGraph resolves a pod to the controller a person recognizes. A pod created by
a CronJob belongs to the CronJob, not to the Job it ran in, so it appears under
`aggregate=cronjob` rather than `aggregate=job`. OpenCost stops at the Job.
</Note>
...
| Parameter | Type | Description |
| --- | --- | --- |
| `window` | string | The period to report, in the same forms `/allocation` accepts. Required. |
| `step` | string | The duration of each returned set. Defaults to the whole window. |
| `filter` | string | Restricts the result, in the syntax `/allocation` uses. An asset carries two fields, `cluster` and `node`, and naming any other is a `400`. |
| `filterTypes` | string | A comma-separated list of `Node` and `Disk`. Defaults to both. |
...
allocated that spend to your workloads.

| Parameter | Type | Description |
| --- | --- | --- |
| `window` | string | The period to report, in the same forms `/allocation` accepts. Required. |
| `aggregate` | string | A comma-separated list of properties to group by. Defaults to `provider,service`. |
| `step` | string | The duration of each returned set. Defaults to the whole window. |
| `accumulate` | boolean | Collapses the range into a single set. |

This route reads billing data rather than cluster data, so it needs the
`focus:read` scope. A key with only `operator:read` reads allocation and assets
and gets `403` here.
