...
- Create a Cloud API key that isn't tied to a cluster:
  `confluent api-key create --resource cloud`.
- Give the key an org-level billing role, **OrganizationAdmin** or **BillingAdmin**. A
  resource-scoped key, or a key without a billing role, returns 403.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Confluent Cloud**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Confluent Cloud needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Confluent Cloud on
...
```bash
focus-exporter --provider confluent --sink costgraph
```

Your Confluent Cloud credential stays where you put it, and only the finished FOCUS
