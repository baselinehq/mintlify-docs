...
- In Helicone, go to **Settings > API keys** and create a key. It starts with
  `sk-helicone-`.
- Note an organization id. Helicone's query API returns no account id, so CostGraph
  uses what you enter as the billing account.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Helicone**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Helicone needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Helicone on a
...
```bash
focus-exporter --provider helicone --sink costgraph
```

Your Helicone credential stays where you put it, and only the finished FOCUS records
