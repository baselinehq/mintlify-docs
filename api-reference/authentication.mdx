...
curl -H "X-API-KEY: $COSTGRAPH_API_KEY" \
     ...
     -X POST https://pricing.baselinehq.cloud/v1/pricing/compute \
...
An invalid key returns a `401` problem document with code `unauthenticated`. On `/v1`, every error is an RFC 9457 problem document; see [Errors](/api-reference/errors).
