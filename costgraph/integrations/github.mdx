...
- Create a token that reads the organization's billing. A fine-grained personal access
  ...
  read** for per-seat data. A classic token needs `repo`, `read:org`, and `copilot`.
...
- If the organization enforces SAML single sign-on, authorize the token for that
  organization. An unauthorized token returns 403.
