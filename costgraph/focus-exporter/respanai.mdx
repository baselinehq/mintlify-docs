...
| `RESPANAI_TENANT_FIELD` | no | Splits spend per downstream tenant into the FOCUS sub-account columns. Set to `customer_identifier` to use each log's customer id, or to a key in each log's `metadata`. See [Guide](/costgraph/focus-exporter/tenancy). |
