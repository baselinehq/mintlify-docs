...
- In the Scaleway console, go to **IAM > API keys** and generate a key.
- Attach a policy granting the **BillingReadOnly** permission set on the organization.
- Copy the secret key. Scaleway displays it only at creation.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Scaleway**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Scaleway needs and marks the
   optional ones.
4. Select **Connect**.
...
```bash
focus-exporter --provider scaleway --sink costgraph
```
