...
- In the DigitalOcean console, go to **API > Tokens** and generate a token with
  **Read** scope. Write access isn't needed.
- A fine-grained token needs `billing:read`, plus `account:read` for CostGraph to label
  the billing account itself.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **DigitalOcean**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the token. Leave the billing account label empty and CostGraph resolves it
   from your account.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads DigitalOcean on a
...
```bash
focus-exporter --provider digitalocean --sink costgraph
```

Your DigitalOcean credential stays where you put it, and only the finished FOCUS
