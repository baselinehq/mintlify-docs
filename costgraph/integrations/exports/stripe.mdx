...
## Downward corrections are held back

Stripe meter events only ever count **up**. There is no event that takes usage
away, so when a cloud provider revises a day's spend downwards - a credit, a
refund, a late discount - CostGraph cannot report the difference.

Rather than overbill quietly or clamp the value, CostGraph **holds the day
back** and records it with the reason `negative_correction_unsupported`. The
day's last reported total stands, and the held day is listed in the meter's
ledger so you can decide what to do: issue a credit note in Stripe, or accept
the difference.

Upward corrections are reported normally.

<Note>
This is a property of Stripe's meter events, not of CostGraph. Platforms whose
events can carry a negative value - Lago, Orb, Metronome, OpenMeter - reconcile
downward revisions automatically.
</Note>

## What lands in Stripe

Each customer-day is one meter event whose payload carries the Stripe customer
id and the amount reported. The event's `identifier` is derived from the day
and the totals, so a run retried within Stripe's deduplication window is
recognised as the same event rather than counted twice.
...
rejection. Stripe bills every event into the customer's **current** period, so
a correction to an old day shows up on the next invoice.
</Note>
...
- A day is held back rather than reported wrong when its spend mixes
  currencies, when a quantity meter meets an unexpected unit, or when the
  correction would be negative.
