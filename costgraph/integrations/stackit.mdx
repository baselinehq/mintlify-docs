...
Send it with the push endpoint, converting each row to a FOCUS record. Use one
`batch_id` for the whole export run and let `row_offset` keep counting across the
parts, so part two starts where part one ended; retry a part with the values it had
the first time. Starting each part back at zero would make later parts overwrite the
rows earlier ones wrote. [Paging a large export](/costgraph/integrations/focus-push#paging-a-large-export)
has the full contract.

Two things in that file we repair rather than reject, so you do not need to
pre-process them: `ChargeDescription` is null on every row, and the service categories
are STACKIT's own names rather than the FOCUS ones.
