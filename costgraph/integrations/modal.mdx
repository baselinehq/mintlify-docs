...
- Run `modal token new` to create a token, then copy the token id and token secret.
- Note the workspace id. The token implies the workspace, and CostGraph uses the id as
  a stable billing account label.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Modal**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what Modal needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads Modal on a
...
```bash
focus-exporter --provider modal --sink costgraph
```

Your Modal credential stays where you put it, and only the finished FOCUS records
