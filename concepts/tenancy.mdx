...
- **AI gateways** attribute each request to the customer that made it. See [AI gateways](/costgraph/integrations/pull/ai-gateways), which also covers labelling requests the gateway could not attribute itself.
- **Pushing your own cost data** lets you set it per row, so two customers on the same product and period stay separate. See [multi-tenancy in FOCUS push](/costgraph/integrations/push/focus-push).
