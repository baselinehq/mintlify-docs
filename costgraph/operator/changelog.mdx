...
<Update label="2026-08-28" description="costgraph-operator v0.5.0">
    ### Breaking Changes
    - **Two objects are renamed and lose the release prefix.** The kubernetes operator's Deployment is now rendered by a shared chart that builds names from a plain value, which cannot include the release name. The service account it references cannot be templated either, so it is pinned.

      | Object | Before | After |
      | --- | --- | --- |
      | Deployment | `<release>-costgraph-operator-kubernetes` | `costgraph-operator-kubernetes` |
      | ServiceAccount | `<release>-costgraph-operator` | `costgraph-operator` |

      `helm upgrade` handles this on its own: the renamed objects are created and the old ones pruned, with no manual step. **Two releases of this chart in one namespace now collide**, where the release prefix previously kept them apart. Install them in separate namespaces.

      The credentials Secret is unchanged and keeps its release-derived name.
    - **Values under `operatorKubernetes` move beneath `deployment`.** A values file using the old paths still parses, but the settings no longer apply and the chart's defaults are used instead. Nothing errors, so check any override you rely on:

      | Before | After |
      | --- | --- |
      | `operatorKubernetes.image.tag` | `operatorKubernetes.deployment.image.tag` |
      | `operatorKubernetes.replicas` | `operatorKubernetes.deployment.replicas` |
      | `operatorKubernetes.resources` | `operatorKubernetes.deployment.resources` |
      | `operatorKubernetes.enabled` | `operatorKubernetes.deployment.enabled` |
      | `operatorKubernetes.nodeSelector` | `operatorKubernetes.deployment.nodeSelector` |
      | `operatorKubernetes.tolerations` | `operatorKubernetes.deployment.tolerations` |
      | `operatorKubernetes.affinity` | `operatorKubernetes.deployment.affinity` |
      | `operatorKubernetes.podAnnotations` | `operatorKubernetes.deployment.additionalPodAnnotations` |
      | `operatorKubernetes.podLabels` | `operatorKubernetes.deployment.podLabels` |
      | `operatorKubernetes.securityContext` | `operatorKubernetes.deployment.securityContext` |
      | top-level `imagePullSecrets` | `operatorKubernetes.deployment.imagePullSecrets` |

      The top-level `imagePullSecrets` still applies to everything else in the chart, but no longer
      reaches this Deployment, so a private operator image needs it repeated under the alias.

      `operatorKubernetes.config.operatorVersion` is removed. `OPERATOR_VERSION` has always been
      taken from the image tag, so the value was inert.

      The top-level `global`, `rbac` and `serviceAccount` values are unchanged.

    ### Updates
    - `operatorPrometheus` is unchanged in both its values and its rendering
    - The flowtrace DaemonSet, the ClusterRole and its bindings, the credentials Secret and every bundled exporter are unchanged
</Update>
