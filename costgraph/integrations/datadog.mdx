...
- In Datadog, go to **Personal Settings > Access Tokens** and create a service access
  token with the `usage_read` and `billing_read` scopes. Set the expiration to
  **Never** so the connection doesn't lapse. The cost endpoints are parent-org only,
  so create the token as a parent-org admin.
- Note your Datadog site if it isn't US1, for example `datadoghq.eu`.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **Datadog**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the access token, and the Datadog site if your account isn't on US1.
4. Select **Connect**.
