...
| Variable | Required | Secret | Description |
| --- | --- | --- | --- |
| `DATADOG_ACCESS_TOKEN` | yes | yes | Datadog access token scoped to usage_read and billing_read (Personal Settings -> Access Tokens): a service access token (ddsat_) for an unattended sync, or a personal access token (ddpat_). For an unattended sync use a service access token with a defined expiry, and rotate it before it lapses. It must belong to the parent organization. |
