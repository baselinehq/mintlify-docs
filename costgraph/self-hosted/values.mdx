...
There is no shared image value. Pinning one component never moves another.
...
`<component>.enabled` gates the component. A disabled component renders nothing
at all rather than rendering and being filtered.
...
than by an `enabled` key of their own. Everything else about their shape lives
in their own top-level block, like every other component.
...
connection strings, credentials, base URLs. Anything more than one component
reads has to live there. In
...
<Warning>
  `global.imagePullSecrets` does not reach every component. Each component names
  its pull secret in its own `<component>.deployment.imagePullSecrets`.
...
<ParamField body="global.redis.bundled.enabled" type="boolean">
  ...
  the `redis` component, so leaving it off renders nothing.
</ParamField>
...
<ParamField body="global.imagePullSecrets" type="array">
  ...
  every component - see the render-time check above.
</ParamField>

## Ingress

The Ingress fans one host out to several Services by port number.
...
<ParamField body="vmalert" type="object">
  ...
  It has no `service:` or `rbac:` block. A Service is rendered for each of the
  two Deployments, so `/api/v1/rules` is reachable even when a rule produces no
  samples, and vmalert runs with no ServiceAccount of its own.
</ParamField>
...
| `<component>.image.*` (component root) | `<component>.deployment.image.*` |
...
| `global.namespace` | removed - it never had an effect. The release namespace is used. |

`flowtrace.maxUnavailable` and `operatorPrometheus.scrapeTargets` stay at
component level. `enabled` stays at component level on all four;
