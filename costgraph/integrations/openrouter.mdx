...
- Create a management key at
  [openrouter.ai/settings/management-keys](https://openrouter.ai/settings/management-keys).
  An inference key, which starts with `sk-or-v1-`, returns
  `403 Only management keys can perform this operation`.
- Note an organization id. The activity API returns no account id, so CostGraph uses
  what you enter as the billing account.

## Let CostGraph pull

1. Open **Settings > Integrations** and pick **OpenRouter**.
2. Name the connection and pick the tenant these charges belong to.
3. Enter the values you copied. The form lists what OpenRouter needs and marks the
   optional ones.
4. Select **Connect**.
...
Rather than build that request yourself, run our exporter. It reads OpenRouter on a
...
```bash
focus-exporter --provider openrouter --sink costgraph
```

Your OpenRouter credential stays where you put it, and only the finished FOCUS records
