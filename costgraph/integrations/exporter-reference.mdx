...
| `BUILDKITE_ORG` | yes | no | Organization slug whose seats are exported the org in the buildkite.com/ path. |
...
| `GITHUB_ORG` | yes | no | Organization login whose billing usage is exported the org in the github.com/ path. |
...
| `GRAFANA_CLOUD_ORG` | yes | no | Grafana Cloud organization slug the org in the grafana.com/orgs/ path whose billed usage is exported. |
...
| `OBJECTSTORE_KIND` | no | no | Which storage this is: s3 (the default), azure, or local for a directory the exporter can read. Inferred from the endpoint where it can be. |
| `OBJECTSTORE_LAYOUT` | no | no | How the provider arranges files: manifest when it publishes one naming its data files, latest-run when it rewrites a billing period on every run, or flat (the default) to read every file under the prefix. |
